...
# 1) run make once to create and update the po/performanceoverlay.pot
# 2) copy the template file and set the name to the language you want to
#    translate to:   cp po/performanceoverlay.pot po/en.po
# 3) edit the po file
...
APP_ID=performanceoverlay.liu-xiao-guo
...
po/performanceoverlay.pot: $(QMLJS_FILES)
	mkdir -p $(CURDIR)/po && xgettext -o po/performanceoverlay.pot --qt --c++ --add-comments=TRANSLATORS --keyword=tr --keyword=tr:1,2 $(QMLJS_FILES) --from-code=UTF-8
